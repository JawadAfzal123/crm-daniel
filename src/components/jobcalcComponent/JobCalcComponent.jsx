...
const JobCalculator = () => {
  ...
  const calculateCommissionAmount = () => {
    return (
      parseFloat(formData.commissionableamount || 0) *
      (parseFloat(formData.commissionpercentage || 0) * 0.01)
    );
  };
  
  const calculateCaulkingAndScrews = () =>
    parseFloat(formData.materialcost || 0) * 0.05;

  const calculateContractTotal = () =>
    Math.round(
      calculateMaterialTax() + parseFloat(formData.commissionableamount || 0)
    );
  ...
  const calculateTotalCost = () => {
    ...
    return (
      parseFloat(formData.additionalstructurecost || 0) + 
      parseFloat(formData.constructiontypecost || 0) + 
      parseFloat(formData.constructiontypecost || 0) +
      parseFloat(formData.materialcost || 0) + 
      commissionAmount +
      caulkingAndScrews 
  ...
  const calculateTotalProfit = () => {
    const totalCost = calculateTotalCost();
    return parseFloat(formData.commissionableamount || 0) - totalCost;
  };

  const calculateProfitPercentage = () => {
    const totalProfit = calculateTotalProfit();
    return (
      (totalProfit / (parseFloat(formData.commissionableamount || 0) ))
    );
  };
    ...
    <div>
      ...
      <div className="bg-white overflow-x-auto shadow-md rounded-lg p-4 mt-4">
        ...
        <div className="mt-5 bg-gray-100 p-4 rounded-lg">
          ...
          <p>Total Profit: ${calculatedValues.totalprofit}</p>
        </div>
      </div>
    </div>
  );
